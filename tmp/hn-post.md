...
- **Context-efficient.** The same logic that takes 100 lines of Python can be 30 tokens of NAIL structure. More logic fits in one context window.

**Phase 2 experiment results:** We had Claude and GPT-4o implement 5 functions (is_even, abs_val, max_of_two, clamp, factorial) in both NAIL and Python. NAIL results: 5/5 passed spec verification — the structured format made it nearly impossible to produce type errors or ambiguous code. Python results had the usual issues: missing type annotations, implicit coercion assumptions, no effect tracking.
