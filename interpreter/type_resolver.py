"""Shared type resolution logic for Checker and Runtime.

Both Checker and Runtime need to resolve NAIL type aliases and nested specs.
This module extracts the shared logic to avoid duplication (Issue #95).

The three methods that were virtually identical in both classes:
  - _substitute_params_in_spec
  - _resolve_alias_spec
  - _resolve_type_spec

are now implemented once in TypeResolver.  Each class creates a TypeResolver
with the appropriate error class and delegates to it.
"""
from __future__ import annotations

from .types import NailRuntimeError
...
class TypeResolver:
    ...
    Usage::
        ...
        # In Checker
        resolver = TypeResolver(error_cls=CheckError)

        # In Runtime
        resolver = TypeResolver(error_cls=NailRuntimeError)

    Aliases and caches are passed per-call (they are mutable and may be
    reassigned on the owning class, so storing them here would cause
    stale-reference bugs).
    """

    def __init__(self, error_cls=NailRuntimeError):
        self._error_cls = error_cls

    # ------------------------------------------------------------------
    # substitute_params_in_spec
    # ------------------------------------------------------------------
        ...
        *,
        ...
        if is_generic:
            ...
            if len(provided) != len(type_params):
                raise self._error_cls(
                    f"Generic alias '{alias_name}' requires {len(type_params)} "
                    f"type argument(s), got {len(provided)}"
                )
    ...
    def resolve_type_spec(
        ...
        *,
        ...
        """Recursively resolve a raw type-spec dict, expanding aliases."""
