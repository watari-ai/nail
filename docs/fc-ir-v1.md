...
| フィールド | 型 | 説明 |
|-----------|-----|------|
| `nail_version` | string | 生成に使用した NAIL バージョン |
| `created_at` | string | ISO 8601 形式の生成日時 |
| `source_hash` | string | 生成元ソースファイルの SHA-256 ハッシュ |
| `spec_rev` | string | この仕様書のリビジョン（git commit hash 等） |
...
| `input` | Type | 引数の型定義。**必ず `type: "object"` でなければならない**（FC003）。 |
...
1. `id` の `.` と `-` を `_` に置換
2. その他の provider 禁止文字（空白・`/`・`@` 等）を `_` に置換
3. 先頭が数字の場合は `t_` プレフィックスを付与
4. 全体を小文字に正規化
5. 連続する `_` を1つに圧縮
...
{
  "lossy": {
    "weather.get.input.max_results": ["int.bits", "int.overflow"]
  }
}
```
...
lossy 情報は変換時に `--lossy-report <file>` オプションで JSON ファイルに出力できる。
