...
存在しないキーは省略する。余分なキーを含めてはならない（`annotations` 内の追加キーを除く）。
...
- **`nail fc canonicalize`**: 未知キーを**サイレントに削除してはならない**。未知キーを検出したら変換を中断し、エラーを返す（情報破壊防止のため）

エラーメッセージ例:
```
FC011 ERROR: Unknown key 'timeout' in ToolDef 'weather.get'. Move to 'annotations' or remove before canonicalizing.
```
...
```
name = /^[a-z][a-z0-9_]*$|^t_[a-z0-9_]*$/
```

最終的に name は `/^[a-z0-9_]+$/` に適合しなければならない。適合しない場合は `nail fc check` で **FC002 ERROR** を発生させる。
...
オプションとして、OpenAI の `function.description` に注記を付加する機能を `nail fc export --openai --annotate-effects` で提供する（デフォルト OFF）:
...
`nail fc export --provider <name>` で各 provider の schema 形式に変換できる。

```bash
nail fc export --provider openai   input.json -o openai-tools.json
nail fc export --provider anthropic input.json -o anthropic-tools.json
nail fc export --provider gemini    input.json -o gemini-tools.json
```
...
| **FC011** | ERROR | ToolDef に `annotations` 以外の未知キーが存在 | `Unknown key '{key}' in ToolDef '{id}'. Move to 'annotations' or remove before canonicalizing.` |
...
NAIL source (.nail)
 ...
 fc_ir_v1 JSON        ← 本仕様書が定義するフォーマット
       ...
       │
       ├─► nail fc export --provider openai      → OpenAI tools JSON
       ├─► nail fc export --provider anthropic   → Anthropic tools JSON
       └─► nail fc export --provider gemini      → Gemini tools JSON
```
